                    ...
                    if (descriptor_type) |dt| {
                        switch (dt) {
                            .Device => {
                                ...
                                // AF CHECK@memcpy(S.tmp, dc, dc.len);
                            ...
                            .Config => {
                                ...
                                // AF CHECK @memcpy(&S.tmp[used], &cd, cd.len);
                                used += cd.len;
                                ...
                                if (setup.length > used) {
                                    ...
                                    // AF CHECK @memcpy(&S.tmp[used], &id, id.len);
                                    used += id.len;
                                    ...
                                    if (usb_config.?.hid) |hid_conf| {
                                        ...
                                        // AF CHECK @memcpy(&S.tmp[used], &hd, hd.len);
                                        used += hd.len;
                                    }
                                    ...
                                    for (usb_config.?.endpoints[2..]) |ep| {
                                        ...
                                        // AF CHECK @memcpy(&S.tmp[used], &ed, ed.len);
                                        used += ed.len;
                                    }
                                }
                            ...
                            .String => {
                                ...
                                const bytes = StringBlk: {
                                    if (i == 0) {
                                        ...
                                        S.tmp[1] = 0x03;
                                        // AF CHECK @memcpy(&S.tmp[2], s, s.len);
                            ...
                            .DeviceQualifier => {
                                ...
                                // AF CHEKC @memcpy(S.tmp, &data, data.len);
                            ...
                            if (_hid_desc_type) |hid_desc_type| {
                                switch (hid_desc_type) {
                                    .Hid => {
                                        ...
                                        const hd = hid_conf.hid_descriptor.serialize();
                                        // AF CHEKC @memcpy(S.tmp, &hd, hd.len);
